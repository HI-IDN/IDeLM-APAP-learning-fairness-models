"""
This module contains the AllocationModel class which is used to create and solve an optimization
model for doctor scheduling.

The AllocationModel class uses the Gurobi optimization library to create a model, set decision
variables, constraints, and an objective function. The model aims to optimize the allocation of
doctors to various roles and orders in a way that maximizes equity among doctors in terms of work
distribution, minimizes the maximum number of times any doctor is assigned the "Cardiac" or
"In Charge" roles, and prioritizes doctors in 'call_and_late_call_doctors' list to be "In Charge".

The AllocationModel class has methods to set up the model, solve it, and retrieve the solution.
It also includes methods to add various constraints to the model, such as ensuring each order on
a day is assigned to at most one doctor, distributing orders among doctors in an equitable manner,
and ensuring no doctor can serve both as a "Cardiac" and a "Charge" doctor on the same day.

This module also imports and uses utility functions from the `data.utils` module and constants
from the `data.schedule` module.

Classes:
    AllocationModel: A class used to represent the doctor scheduling optimization model.
...
Constants:
    ADMIN_POINTS:   A constant imported from the `data.schedule` module representing the additional
                    points given to doctors with administrative duties.
    Assignment:     A class imported from the `data.schedule` module representing an assignment of
                    a doctor to an order.
"""

from gurobipy import Model, GRB  # pylint: disable=no-name-in-module
...
class AllocationModel:
    """
    This class represents the doctor scheduling optimization model.

    The AllocationModel class uses the Gurobi optimization library to create a model, set decision
    variables, constraints, and an objective function. The model aims to optimize the allocation of
    doctors to various roles and orders in a way that maximizes equity among doctors in terms of
    work distribution, minimizes the maximum number of times any doctor is assigned the "Cardiac"
    or "In Charge" roles, and prioritizes doctors in 'call_and_late_call_doctors' list to be
    "In Charge".

    The AllocationModel class has methods to set up the model, solve it, and retrieve the solution.
    It also includes methods to add various constraints to the model, such as ensuring each order on
    a day is assigned to at most one doctor, distributing orders among doctors in an equitable
    manner, and ensuring no doctor can serve both as a "Cardiac" and a "Charge" doctor on the same
    day.

    Attributes:
        m (Model):           The Gurobi model.
        data (DataHandler):  Known parameters.
        solution (dict):     Dictionary containing the solution to the model.
        x (dict):            Decision variables for the model.
        central_value (Var): Represents the target value for average orders across all doctors.
        y (dict):            Binary variable indicating whether the doctor's total order deviates
                             from the central value.
        z (dict): Binary variables to indicate if a doctor is in charge for a particular day.
        w (dict): Binary variables to indicate if a doctor is the Cardiac doctor for the day.
        max_w (Var): Represents the maximum number of times any doctor is assigned "Cardiac" role.
        max_z (Var): Represents the maximum number of times any doctor is in charge.
        max_wz (Var): Indicates the maximum times any doctor holds both "Cardiac" and "Charge"
                      roles over a week.
        obj_var (dict): Variables representing different objectives of the model.
    """
    ...
    def __init__(self, data):
        ...
        self.central_value = self.m.addVar(lb=0.0, vtype=GRB.CONTINUOUS,
                                           name="TargetTotalOrderValue_central_value")
        """
        central_value:
        Represents the target value for average orders across all doctors.
        It is estimated as the total orders divided by the total working days.
        """

        # Binary variables to capture specific doctor conditions
        self.epsilon_values = [1, 0.5, 0.2]
        self.y = {eps: self.m.addVars(self.data.doctors, vtype=GRB.BINARY,
                                      name=f"DoctorCondition_y_{eps}") for eps in
                  self.epsilon_values}
        """
        y[eps][doctor]: 
        Binary variable indicating whether the doctor's total order deviates from the central value 
        by more than 'eps' units. It is set to 1 if the deviation is more than 'eps' units, and 0 
        otherwise. This variable is used to track and enforce the desired level of order equity for 
        each doctor at different deviation thresholds (represented by 'eps').
        """

        # Binary variables to indicate if a doctor is in charge for a particular day
        self.z = self.m.addVars(self.data.staff.charge_doctors, self.data.weekdays,
                                vtype=GRB.BINARY,
        ...
        self.w = self.m.addVars(self.data.staff.cardiac_doctors, self.data.weekdays,
                                vtype=GRB.BINARY,
        ...
        """
        max_wz:
        Indicates the maximum times any doctor holds both "Cardiac" and "Charge" roles over a week.
        """
    ...
    def solve(self):
        ...
        if (self.m.status == GRB.OPTIMAL or
    ...
    def _get_solution(self):
        ...
        for day in self.data.weekdays:
            ...
            for order in self.data.orders[day]:
                for doctor in self.data.working[day]:
                    if self.x[doctor, day, order].X > 0.5:
                        # If this doctor is assigned to this order on this day
        ...
        # Sanity checks
        for day in self.data.weekdays:
            assert len(chrg[day]) == 1, f"Only 1 doctor for charge on {day}: {chrg[day]}"
            assert len(diac[day]) == 1, f"Only 1 doctor for cardiac on {day}: {diac[day]}"
            chrg[day] = chrg[day][0]
            diac[day] = diac[day][0]

        total_points = {doctor: int(total.getValue()) for doctor, total in self.total_order.items()}
        preassigned_points = {doctor: self.data.get_points_per_doctor(doctor, add_assigned=False)
                              for doctor in self.data.doctors}
        points = {doctor: [total_points[doctor], preassigned_points[doctor]]
                  for doctor in sorted(self.data.doctors)}
        ...
        return {'Params': self.solution['Params'],
                'Whine': whine, 'Charge': chrg, 'Cardiac': diac, 'Points': points,
                'Target': target, 'Objective': obj}
    ...
    def _set_objective(self):
        ...
        2. Minimize the maximum occurrences any doctor is assigned "Cardiac" or "In Charge" roles.
        ...
        - `gamma` for increasing the likelihood of doctors from 'call_and_late_call_doctors' to be
           "In Charge" on specific days.

        """
        # Weights for different objectives
        alpha = 1  # Weight for the equity objective
        alpha_weights = {1: 1, 0.5: 0.5,
                         0.2: 0.2}  # Weight for the equity sub-objectives per epsilon range
        ...
        self.m.addConstr(
            self.obj_var['equity'] == sum(
                weight * sum(self.y[eps][doctor] for doctor in self.data.doctors)
                for eps, weight in alpha_weights.items()
            )
        )
    ...
    def _add_preassigned_doctor_constraints(self):
        ...
        1.  This method ensures that each pre-assigned doctor is allocated to exactly one peel-off
            position per pre-assigned day.
        2.  It also directly assigns each pre-assigned doctor to their specific peel-off position on
            each pre-assigned day.
        """
        self.m.addConstrs(
            (sum(self.x[doctor, day, order] for order in self.data.orders[day]) == 1
             for day in self.data.days for doctor in list(self.data.preassigned[day].values())),
            name="unique_peel_off_position_per_preassigned_doctor"
        )
        ...
        for day, order_doctor_dict in self.data.preassigned.items():
            for order, doctor in order_doctor_dict.items():
                self.m.addConstr(self.x[doctor, day, order] == 1,
                                 name=f"preassigned_doctor_{doctor}_{day}_{order}")
    ...
    def _add_constraints_for_doctor_order_equity(self):
        ...
        bigM = len(self.data.days) * len(self.data.orders)  # pylint: disable=invalid-name

        # Constraints to ensure the mean order for each doctor is around the central_value.
        # If a doctor's mean order isn't within the range (central_value - eps, central_value + eps)
        ...
        for eps in self.epsilon_values:
            self.m.addConstrs(
                (self.total_order[doctor] / self.data.working_doctors[doctor]['Weekdays'] - (
                        self.central_value - eps)
                 >= -bigM * (1 - self.y[eps][doctor])
                 for doctor in self.data.doctors if
                 self.data.working_doctors[doctor]['Weekdays'] > 0),
                name=f"lower_bound_order_constraint_{eps}"
            )
            self.m.addConstrs(
                ((self.central_value + eps) - self.total_order[doctor] /
                 self.data.working_doctors[doctor]['Weekdays']
                 >= -bigM * (1 - self.y[eps][doctor])
                 for doctor in self.data.doctors if
                 self.data.working_doctors[doctor]['Weekdays'] > 0),
    ...
    def _add_cardiac_constraints(self):
        ...
        2. Calculate the total occurances each doctor is assigned the "Cardiac" role over the week.
        3. Ensure that the total number of times each doctor is assigned the "Cardiac" role does not
           exceed the maximum allowed.

        Uses:
        - w[doctor, day] binary: Represents whether a doctor is assigned "Cardiac" role that day.
        - max_w: Variable for the maximum number of times any doctor is assigned the "Cardiac" role.

        Returns:
            dict: Dictionary containing total number of times each doctor is assigned the "Cardiac"
                  role over the week.
        """
        ...
        total_in_cardiac = {
            doctor: sum(
                self.w[doctor, day] for day in self.data.weekdays if
                doctor in self.data.call_doctors[day])
    ...
    def _add_charge_constraints(self):
        ...
        1. Ensure that only one doctor from potential charge doctors has the "Charge" role each day.
        2. Ensure that if a doctor is in the "Charge" role, they must have the corresponding order
           for that day.
        3. Calculate the total times each doctor is in the "Charge" role over the week.
        4. Ensure that the total number of times each doctor is in the "Charge" role does not exceed
           the maximum allowed.

        Uses:
        - z[doctor, day] binary: Represents whether a doctor is the "Charge" doctor on a given day.
        - max_z: Variable for the maximum number of times any doctor is in the "Charge" role.
        - x[doctor, day, order] binary: Whether a doctor is assigned a specific order on a day.

        Returns:
            dict: Dictionary containing total number of times each doctor is assigned the "Charge"
                  role over the week.
        """
        ...
        self.m.addConstrs(
            (self.x[doc, day, self.data.charge_order[day]] >= self.z[doc, day]
             for day in self.data.weekdays for doc in self.data.staff.charge_doctors if
             doc in self.data.Whine[day]),
            name="charge_order_constr"
        )

        # Calculate total times each doctor is in charge over the week
        total_in_charge = {
            doctor: sum(self.z[doctor, day] for day in self.data.weekdays) for doctor in
            self.data.staff.charge_doctors
        }
    ...
    def _add_no_cardiac_and_charge_dual_role_constraints(self, total_in_cardiac, total_in_charge):
        """
        Add constraints to ensure that a doctor cannot simultaneously assume the "Cardiac" and
        "Charge" roles on the same day.
        ...
        3. Ensure that the total combined roles (both "Cardiac" and "Charge") each doctor assumes
           over the week does not exceed the maximum allowed.

        Parameters:
        - total_in_cardiac: Dictionary containing the total number of times each doctor is assigned
                            the "Cardiac" role over the week.
        - total_in_charge:  Dictionary containing the total number of times each doctor is assigned
                            the "Charge" role over the week.

        Uses:
        - w[doctor, day] binary: Represents whether a doctor is the "Cardiac" role on a given day.
        - z[doctor, day] binary: Represents whether a doctor is the "Charge" doctor on a given day.
        """
        ...
        # Ensure that doctors in the common_doctors set cannot take on both roles simultaneously
        self.m.addConstrs(
            (self.w[doctor, day] + self.z[doctor, day] <= 1 for doctor in common_doctors for day in
             self.data.weekdays),
        ...
        self.m.addConstrs(
            (total_in_charge[doctor] + total_in_cardiac[doctor] <= self.max_wz for doctor in
             common_doctors),
            name="max_combined_roles")

    def _calculate_total_order(self):
        """
        Calculate the total order for each doctor based on their assignments across all days and
        orders. Adjust the total_order values based on administrative duties.

        Attributes:
            total_order (dict): Contains the total order count for each doctor.
            x (dict): A multi-key dictionary representing assignment of doctors to days and orders.
    ...
    def save(self, filename):
        ...
        data['Solution'] = {
            ...
            'Charge': [self.solution['Charge'][day] if day in self.data.weekdays else None for day
                       in self.data.days],
            'Cardiac': [self.solution['Cardiac'][day] if day in self.data.weekdays else None for day
                        in self.data.days],
    ...
    def debug_constraints(self):
        ...
        for day, doctors_list in self.data.potential_charge_doctors.items():
            if day in self.data.weekdays and len(doctors_list) < 1:
                raise ValueError(f"There are <1 doctor for day {day} in 'potential_charge_doctors'")

        for day, doctors_list in self.data.potential_cardiac_doctors.items():
            if day in self.data.weekdays and len(doctors_list) < 1:
                raise ValueError(
                    f"There are <1 doctor for day {day} in 'potential_cardiac_doctors'")

        # If it's exactly 1 doctor for both, check if they are different
        for day in self.data.weekdays:
            if len(self.data.potential_charge_doctors[day]) == 1 and len(
                    self.data.potential_cardiac_doctors[day]) == 1:
                if self.data.potential_charge_doctors[day][0] == \
                        self.data.potential_cardiac_doctors[day][0]:
                    raise ValueError(
        ...
        for day in self.data.weekdays:
            ...
            if len(unique_doctors) < 2:
                raise ValueError(
                    f"The union of doctors from both 'potential_charge_doctors' and "
                    f"'potential_cardiac_doctors' for day {day} is less than 2")
